...
register_nav_menus( array(
	'sub' => __('Sub Navigation', 'cTpl_pc007_blue'),
	)
);
...
add_filter( 'wp_list_categories', 'cTpl_pc007_blue_list_categories', 10, 2 );
function cTpl_pc007_blue_list_categories( $output, $args ) {
  $output = preg_replace('/<\/a>\s*\((\d+)\)/',' ($1)</a>',$output);
  return $output;
}

add_filter( 'get_archives_link', 'cTpl_pc007_blue_archives_link' );
function cTpl_pc007_blue_archives_link( $output ) {
...
/*	Add admin CSS
/*---------------------------------------------------------*/
function cTpl_pc007_blue_admin_css(){
	$adminCssPath = get_template_directory_uri().'/cTpl_admin.css';
	wp_enqueue_style( 'theme', $adminCssPath , false, '2012');
}
add_action('admin_head', 'cTpl_pc007_blue_admin_css', 11);


/*	Display navigation to next/previous pages when applicable
/*---------------------------------------------------------*/
function cTpl_pc007_blue_content_nav( $nav_id ) {
	global $wp_query;
